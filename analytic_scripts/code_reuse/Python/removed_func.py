...
def entirelyRemoved(complete_def, in_removed, indent, func_name, arg_cnt, removed_func_list):
    if DEBUG:
        print '\t(Entirely removed function: %s %d)' %(func_name,arg_cnt)
    removed_func_list.append([func_name,arg_cnt])
...
def removedFunctions(diff_list):
    ...
    for line in diff_list:
                ...
                if matched_def:
                    ...
                    if match_res:
                        ...
                        if complete_def:
                            if DEBUG:
                                print line
                            complete_def, in_removed, indent, func_name = entirelyRemoved(complete_def, in_removed, indent, func_name, arg_cnt, removed_func_list)
        ...
        last_cnt -= 1
    return removed_func_list

def parseArgsOfInvocation(arg_string, matched_name, analyzed_line, func_name, line_num, removed_func_list, removed_invocations):
    cleaned_str, end_definition = parseArgs(analyzed_line)
    arg_string += cleaned_str
    if end_definition:
        if DEBUG:
            print arg_string, simpleCountArgs(arg_string)
        if [func_name, simpleCountArgs(arg_string)] in removed_func_list:
            if DEBUG:
...
def removedInvocations(diff_list, removed_func_list):
    ...
    for line in diff_list:
        if line.startswith('--- '):
            ...
            if line.startswith('-') and is_py_file:
                ...
                if not purified_line.lstrip().startswith('def'):
                    if matched_name:    # match the rest line(s) of the function call
                        ...
                        if analyzed_line.endswith('\\'):
                            ...
                            unparsed_line = ''
                            arg_string, matched_name = parseArgsOfInvocation(arg_string, matched_name, analyzed_line, func_name, line_num, removed_func_list, removed_invocations)
                    else:                   # match the first line of a function call
                        for func in removed_func_list:
                            func_name = func[0]
                            func_args = func[1]
                            if re.search('%s\s*?\((.*?)' %func_name, line):
                                if DEBUG:
                                    print line
                                matched_name = True
